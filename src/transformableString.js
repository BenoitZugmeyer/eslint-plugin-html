...
export default class TransformableString {
...
function lineStarts(str) {
  const result = [0]
  LINE_ENDING_RE.lastIndex = 0
  while (true) {
    const match = LINE_ENDING_RE.exec(str)
    if (!match) break
    result.push(LINE_ENDING_RE.lastIndex)
  }
  return result
}
